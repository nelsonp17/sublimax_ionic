    ...
    <div id="container">
            ...
            <ion-card-content style="display: flex;">
                <div style="margin: auto;">
                    <ion-avatar aria-hidden="true" slot="start" style="height: 40px; width: 10px;">
                        <ion-icon aria-hidden="true" :icon="personCircleOutline" style="font-size: 2em; color:#000; margin-left: .5em;"/>
                    </ion-avatar>
                    <ion-label>Mis Datos</ion-label>
                </div>
                <div style="margin: auto;">
                    <ion-avatar aria-hidden="true" slot="end" style="height: 40px; width: 10px;">
                        <ion-icon aria-hidden="true" :icon="call" style="font-size: 2em; color:#000; margin-left: .5em;" />
                    </ion-avatar>
		...
		<div class="mt-2" v-if="admin==true">
				...
				<ion-card-content>
					...
					<div style="clear: both; height: 40px;">
						<div style="float:left;">
							<ion-label><p style="line-height: 40px; color: #212121">Agregar un producto</p></ion-label>
						</div>
			...
			<ion-list :inset="true" style="margin-left: 0; margin-right: 0; padding:0" class="myList mt-2">
				<router-link to="/environment">
					<ion-item :button="true" v-if="VITE_MODE_DEVELOPER=='true'">
        ...
        <div class="mt-2">
            ...
            <ion-list class="myList" lines="inset" style="margin-top: 0; padding-top:0">
                <ion-item>
                    <ion-label>Términos y condiciones <ion-icon aria-hidden="true" :icon="chevronForward" style="float: right" /></ion-label>
                </ion-item>
				<ion-item>
                    <ion-label>Seguridad <ion-icon aria-hidden="true" :icon="chevronForward" style="float: right" /></ion-label>
                </ion-item>
				<ion-item>
			...
			<ion-list class="myList" lines="none" style="margin-top: 0; padding-top:0">
				<ion-item>
                    <ion-label><p class="color: #BDBDBD!important">Versión {{ VITE_VERSION_APP }} </p></ion-label>
                </ion-item>
            </ion-list>
			<ion-list class="myList" lines="none">
				<ion-item>
                    <ion-label>Cerrar Sessión <ion-icon aria-hidden="true" :icon="exitOutline" style="float: right" /></ion-label>
                </ion-item>
            </ion-list>
        </div>
    </div>
...
<script setup lang="ts">
  ...
  import { IonCard, IonCardContent, IonIcon, IonItem, IonList, IonButton, IonNote, IonCardHeader, IonCardSubtitle, IonCardTitle, IonAvatar, IonLabel } from '@ionic/vue';
  ...
  async function getCountRow(){
	...
	tablesCount.value.environment = environment.length > 0 ? environment[0].count : 0;
	tablesCount.value.subcategory = environment.length > 0 ? environment[0].count : 0;
	tablesCount.value.category = environment.length > 0 ? environment[0].count : 0;
	tablesCount.value.products = environment.length > 0 ? environment[0].count : 0;
